...
<div class="headermenu">
    ...
    <?php  if(!vp_metabox('photology_masonry_blog.hide_filter')) { ?>
        <!-- filter header -->

        <div class="blogfilter topleftmenu">

            <div class="blogfilterbutton" data-text="<?php esc_attr_e("Blog Sort &amp; Filter", "photology-themes") ?>">
                <span><?php _e("Blog Sort &amp; Filter", "photology-themes") ?></span>
            </div>

            <div class="blogfilterlist">
                <h3><?php _e("Sort blog by :", "photology-themes") ?></h3>
                <ul class="blogsortul" data-title="<?php esc_attr_e("Sort by", "photology-themes") ?>">
                    <li data-type ="sort" data-sortby="popular"><?php _e("Popularity", "photology-themes") ?></li>
                    <li data-type ="sort" data-sortby="date"><?php _e("Date", "photology-themes") ?></li>
                </ul>
                <?php if( vp_metabox('photology_blogcontent.toggle_filtering') !== '1') { ?>
                    <h3><?php _e("Filter blog category :", "photology-themes") ?></h3>
                    <ul class="blogfilterul" data-title="<?php esc_attr_e("Filter by", "photology-themes") ?>">
                        <li data-type="category" data-filter=""><?php _e("All Category", "photology-themes") ?></li>
                        <?php
                        foreach($categories as $category)
                        {
                            echo "<li data-type='category' data-filter='". esc_attr($category->term_id) ."'>" . esc_html($category->name) . "</li>";
                        }
                        ?>
                    </ul>
                <?php } ?>
...
<div class="blog-masonry-wrapper">
    ...
    <?php
        ...
        if ( $query->have_posts() ) {
            ...
            {
                $query->the_post();
                get_template_part('fragment/masonry-blog-post-summary');
            }
        } else {
            echo
					...
					<article class='article-masonry-box'>
						<div class='article-masonry-wrapper clearfix'>
							<div class='pagetext'>
								...
								" . __('Post Not Found','photology-themes') . "
								</span>
							</div>
						</div>
					</article>
    ...
    <div class="bloginputfilter">
        <form>
            <input type="hidden" name="blogid" value="<?php echo esc_attr(get_the_ID()) ?>"/>
